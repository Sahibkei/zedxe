...
/**
 * Handle Vol-Momo cell distribution requests.
 * @param request - Incoming GET request.
 * @returns Histogram and summary stats for the selected cell.
 */
export async function GET(request: NextRequest) {
    ...
    const j = parsed.data.j;
    if (i < 0 || j < 0 || i >= bins || j >= bins) {
        return errorResponse(400, "BAD_REQUEST", "Invalid bin indices", { i, j });
    }
    ...
    let analysis = getCachedVolMomoAnalysis(cacheKey);
    ...
    if (!analysis) {
        ...
        setCachedVolMomoAnalysis(cacheKey, analysis);
    }

    const values = analysis.samples
        .filter((sample) => sample.binI === i && sample.binJ === j)
        .map((sample) => sample.forwardReturn);

    const stats = summarizePercentiles(values);
    const histogram = buildHistogram(values, 30);
    const pWin =
