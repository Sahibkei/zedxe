...
const computeTotalReturnPct = (points: PerformancePoint[]): number | null => {
    ...
    const firstIdx = points.findIndex((p) => Number.isFinite(p.value) && p.value !== 0);
    const lastIdx = [...points].reverse().findIndex((p) => Number.isFinite(p.value) && p.value !== 0);

    if (firstIdx === -1 || lastIdx === -1) return null;

    const first = points[firstIdx].value;
    const last = points[points.length - 1 - lastIdx].value;
